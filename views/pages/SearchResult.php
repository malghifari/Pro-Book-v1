...
class SearchController  {
    public static function search(){
        ...
        $query = "SELECT book.`id-book`,avatar, title, author, description, AVG(rating) AS avgrating, COUNT(username) AS user  
        ...
        <?php include Config::DOCUMENT_ROOT . "/views/includes/Header.php"?>
            <div class = "container">
                <?php
                    ...
                    echo "<h1>Search Result<span class='result'>found <u>".$sum."</u> result(s)</span></h1>";
                    if ($sum == 0) {
                        echo "Not Found";
                    } else {
                        $i = 0;
                        while($i < sizeof($result)) {
                            ...
                            <div><img src='../../public/img/". $row["avatar"] ."' width='90px' height='110px' align='left'>
                                ...
                                <b class='author'>" . $row["author"] . "- ".number_format((float)$row["avgrating"],1,'.','')."/5.0 (".$row["user"]." votes)
